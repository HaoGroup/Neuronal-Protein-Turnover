...
(function($) {
	$.fn.fancyTable = function(options) {
		var settings = $.extend({
			inputStyle: "",
			inputPlaceholder: "Search (Protein, Gene, Description, or Peptides)...",
			pagination: false,
			paginationClass: "btn btn-light",
			paginationClassActive: "active",
			pagClosest: 3,
			perPageOpts: [10,20,30,50,100],
			perPage: 10,
			sortable: true,
			searchable: true,
			matchCase: false,
			...
			onUpdate: showFinalTable,
			sortFunction: function(a, b, fancyTableObject, rowA, rowB, charta, chartb){
				if(charta!=chartb){ return( (charta>chartb)?1:-1 ); } // chart = 'x' should always be on the bottom
				if(a==b && rowA && rowB){
		...
		this.tableUpdate = function (elm) {
			...
			if(settings.pagination){
				...
				var paginationElementLeft = $("<div>",{style:"float:left"})
				var paginationElementRight = $("<div>",{style:"float:right"})
				for(var n=1; n<=elm.fancyTable.pages; n++){
					if(n==1 || (n>(elm.fancyTable.page-(settings.pagClosest+1)) && n<(elm.fancyTable.page+(settings.pagClosest+1))) || n==elm.fancyTable.pages){
						...
						if(n==elm.fancyTable.pages && elm.fancyTable.page<(elm.fancyTable.pages-settings.pagClosest-1)){
							paginationElementLeft.append($("<span>...</span>"));
						}
						paginationElementLeft.append(a);
				...
				var gopage = $("<input>",{
					...
					"placeholder": "##",
					style:"width:60px;box-sizing:border-box;"+settings.inputStyle
				}).bind("keydown",function(e){
					if(e.keyCode==13){
						var thisval = parseInt($(this).val());
						if (thisval>=0 && thisval <= elm.fancyTable.pages) { 
							elm.fancyTable.page = thisval;
							instance.tableUpdate(elm);
						}
					}
				});
				paginationElementLeft.append("<span>&nbsp;&nbsp;Goto: </span>");
				paginationElementLeft.append(gopage);
				paginationElement.append(paginationElementLeft);

				// Number per page options
				paginationElementRight.append("<span>Rows per page: </span>");
				for(var n=0; n<elm.fancyTable.perPageOpts.length; n++){
					var ppcnt = elm.fancyTable.perPageOpts[n];
					var pgopt = $("<a>",{
						html:ppcnt,
						"data-n": ppcnt,
						style:"margin:0.2em",
						class:settings.paginationClass+" "+((ppcnt==elm.fancyTable.perPage)?settings.paginationClassActive:"")
					}).css("cursor","pointer").bind("click",function(){
						// after changing rows-per-page, original page number could be out of range. Find new page value first
						var newPerPage = $(this).data("n");
						elm.fancyTable.page = 1+parseInt( ( (elm.fancyTable.page - 1)*elm.fancyTable.perPage+1) / newPerPage ) ; 
						elm.fancyTable.perPage = newPerPage;
						instance.tableUpdate(elm);
					});
					paginationElementRight.append(pgopt);
					paginationElement.append(paginationElementRight);
				}
			...
			if(typeof elm.fancyTable.sortColumn !== "undefined" && elm.fancyTable.sortColumn < elm.fancyTable.nColumns){
				...
				var rows = $(elm).find("tbody tr").toArray().sort(
					function(a, b) {
						...
						// adpated for special peptile table column
						var cmpa = $(elma).attr("data-sortvalue") ? $(elma).data("sortvalue") : (elma.attr('class')=='peptideCell')?parseInt(elma.text().split("/")[1]):elma.html();
						// var cmpa = $(elma).attr("data-sortvalue") ? $(elma).data("sortvalue") : (elma.attr('class')=='peptideCell')?parseInt(elma.children("button").html().split("/")[1]):elma.html();
						var cmpb = $(elmb).attr("data-sortvalue") ? $(elmb).data("sortvalue") : (elmb.attr('class')=='peptideCell')?parseInt(elmb.text().split("/")[1]):elmb.html();
						// added to sort all proteins to bottom if no-chart
						var prtncharta = $(a).find("td.chart").eq(0).text();
						var prtnchartb = $(b).find("td.chart").eq(0).text();
						if(elm.fancyTable.sortAs[elm.fancyTable.sortColumn] == 'case-insensitive') {
							cmpa = cmpa.toLowerCase();
							cmpb = cmpb.toLowerCase();
						}
						return settings.sortFunction.call(this,cmpa,cmpb,elm.fancyTable,a,b,prtncharta,prtnchartb);
					}
				);
		...
		this.each(function() {
			...
			elm.fancyTable = {
				...
				perPageOpts : settings.perPageOpts,
			...
			if(settings.sortable){
				...
				$(elm).find("thead th").each(function() {
					elm.fancyTable.sortAs.push(
						...
						// ($(this).data('sortas')=='peptideSpecial') ? 'peptideSpecial' :
