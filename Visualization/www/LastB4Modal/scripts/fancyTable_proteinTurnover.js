			...
			if(typeof elm.fancyTable.sortColumn !== "undefined" && elm.fancyTable.sortColumn < elm.fancyTable.nColumns){
				...
				var rows = $(elm).find("tbody tr").toArray().sort(
					function(a, b) {
						...
						var cmpa = $(elma).attr("data-sortvalue") ? $(elma).data("sortvalue") : (elma.attr('class')=='peptideCell')?parseInt(elma.children(".peptideN").html().split("/")[1]):elma.html();
						var cmpb = $(elmb).attr("data-sortvalue") ? $(elmb).data("sortvalue") : (elmb.attr('class')=='peptideCell')?parseInt(elmb.children(".peptideN").html().split("/")[1]):elmb.html();
